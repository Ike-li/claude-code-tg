...
| uv | Recommended installer and development tool. | Use `uv tool install "git+https://github.com/Ike-li/claude-code-tg.git"` for public Git installs. The latest tagged release is `v0.8.3`; record `tgcc --version` and the installed commit for repeatable deployments. CI and release automation pin workflow `UV_VERSION` to `0.11.17` before running frozen sync. |
| PyPI | Published as of `0.8.3` (`uv tool install claude-code-tg`). | Git installs remain available for the latest dev build. |
