...
| uv | Recommended installer and development tool. | Use `uv tool install "git+https://github.com/Ike-li/claude-code-tg.git"` for public Git installs. The latest tagged release is `v0.8.4`; record `tgcc --version` and the installed commit for repeatable deployments. CI and release automation pin workflow `UV_VERSION` to `0.11.17` before running frozen sync. |
